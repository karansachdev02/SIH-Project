...
function PrebookingForm({ crop, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (parsedQty > maxQty) {
      setFormErr(`Quantity cannot exceed available amount (${maxQty} ${defaultUnit}).`)
      return
    }
    ...
    try {
      ...
      setFormErr(err?.message || 'Failed to submit booking. Please try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="rounded-2xl border border-emerald-200 bg-emerald-50/50 p-5 space-y-4">
      <div>
        <p className="text-sm font-bold text-emerald-900">Pre-book this Crop</p>
        <p className="text-xs text-emerald-700 mt-0.5">
          Available: <strong>{maxQty} {defaultUnit}</strong>
          {priceStr && <span className="ml-2">· Expected price: <strong>{priceStr}/{defaultUnit}</strong></span>}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1">
            Note to farmer <span className="text-slate-400">(optional, max 500 chars)</span>
          </label>
          <textarea
            value={note}
            onChange={(e) => setNote(e.target.value.slice(0, 500))}
            rows={2}
            placeholder="e.g. Preferred pickup time, quality requirements…"
        ...
        >
          {loading ? (
            ...
            <><ClipboardList size={18} /><span>Submit Pre-booking Request</span></>
          )}
        </button>
      </form>
    </div>
...
function BookingSuccess({ booking, cropName, onNavigate }) {
  const fmt = (val) => {
    const n = Number(val)
  ...
  return (
    <div className="rounded-2xl border border-emerald-300 bg-emerald-50 p-6 space-y-4 text-center">
      ...
      <div>
        <p className="text-lg font-extrabold text-emerald-900">Booking Request Submitted!</p>
        <p className="text-sm text-emerald-700 mt-1">Waiting for the farmer to confirm your request.</p>
      </div>
      <div className="bg-white rounded-xl border border-emerald-200 p-4 text-left space-y-2 text-sm">
        <Row label="Crop"          value={cropName} />
        ...
        <Row label="Booking Ref"   value={<span className="font-mono text-xs text-slate-500">{String(booking.id).slice(-8).toUpperCase()}</span>} />
      </div>
      <button
        type="button"
...
function FarmerReviewsPanel({ farmerId }) {
...
export default function CropDetails({ cropId, onNavigate }) {
  ...
  const fetchCrop = useCallback(async () => {
    if (!cropId) { setError('No crop selected.'); setLoading(false); return }
    ...
    try {
      ...
      if (status === 404)      setError('This crop is no longer available or does not exist.')
      else if (status === 400) setError('Invalid crop reference.')
      else                     setError(err?.message || 'Failed to load crop details. Please try again.')
      setCrop(null)
    } finally {
      setLoading(false)
    }
    ...
    return (
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <div className="py-14 flex flex-col items-center gap-3 text-slate-500">
          ...
          <p className="text-sm font-medium">Loading crop details…</p>
        </div>
      </div>
    )
  }
    ...
    <div className="max-w-lg mx-auto space-y-4">
      ...
      <div className="bg-gradient-to-br from-emerald-800 to-teal-900 text-white rounded-2xl p-6 shadow-xl">
        ...
        {priceStr ? (
          ...
          <p className="mt-3 text-emerald-300 text-sm italic">Price not specified</p>
        )}
        <div className="flex items-center gap-2 mt-3 text-emerald-200 text-sm">
          <Package size={14} aria-hidden="true" />
          <span>Available: <strong className="text-white">{quantity} {quantityUnit || 'quintal'}</strong></span>
        </div>
      </div>
      ...
      <Card className="p-0 divide-y divide-slate-100 overflow-hidden">
        <div className="px-5 py-4">
          <h2 className="text-sm font-bold text-slate-700 uppercase tracking-wide">Crop Details</h2>
        </div>
        <div className="px-5">
          <DetailRow icon={Tag}          label="Crop Type"    value={cropType || null} />
          <DetailRow icon={Package}      label="Quantity"     value={`${quantity} ${quantityUnit || 'quintal'}`} />
          <DetailRow icon={MapPin}       label="Location"     value={locationStr} />
          <DetailRow icon={CalendarDays} label="Harvest Date" value={formatDate(harvestDate)} />
          <DetailRow icon={Info}         label="Description"  value={description || null} />
          <DetailRow icon={CalendarDays} label="Listed On"    value={formatDate(createdAt)} />
        </div>
      </Card>

      {/* Seller info */}
      <Card className="p-5">
        <h2 className="text-sm font-bold text-slate-700 uppercase tracking-wide mb-4">Seller Information</h2>
        <div className="flex items-start gap-3">
          <div className="w-12 h-12 rounded-2xl bg-emerald-100 text-emerald-700 flex items-center justify-center shrink-0">
            <User size={22} aria-hidden="true" />
          </div>
          <div className="space-y-1.5">
            <p className="text-base font-bold text-slate-900">{farmer?.name || 'Farmer'}</p>
            <div className="flex flex-wrap items-center gap-2">
              {isVerified ? (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200 text-xs font-semibold">
                  <CheckCircle2 size={11} />Verified Farmer
                </span>
              ) : (
              ...
              {isNewSeller && (
                <span className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 border border-blue-200 text-xs font-bold uppercase tracking-wide">New Seller</span>
              )}
            </div>
            {!isNewSeller && rating?.average !== null && (
              <div className="flex items-center gap-1 text-sm text-amber-600 font-semibold">
                ...
                <span className="text-slate-400 font-normal text-xs">({rating.count} ratings)</span>
              </div>
            )}
            {isNewSeller && <p className="text-xs text-slate-500">No ratings yet — this is a new seller.</p>}
      ...
      {farmer?.id && (
        <FarmerReviewsPanel farmerId={String(farmer.id)} />
      )}

      {/* Pre-booking section */}
      {confirmedBooking ? (
        <BookingSuccess booking={confirmedBooking} cropName={cropName} onNavigate={onNavigate} />
      ) : isAvailable ? (
        showForm ? (
          <PrebookingForm
            crop={crop}
          ...
          <div className="rounded-2xl border border-emerald-200 p-5 text-center space-y-3 bg-white">
            <p className="text-sm font-bold text-slate-700">Interested in this crop?</p>
            <button
              type="button"
        ...
        <div className="rounded-2xl border-2 border-dashed border-slate-300 p-5 text-center space-y-2">
          ...
          <p className="text-sm font-bold text-slate-600">Crop No Longer Available</p>
          <p className="text-xs text-slate-400">This listing has been closed by the farmer.</p>
        </div>
      )}
    </div>
  )
